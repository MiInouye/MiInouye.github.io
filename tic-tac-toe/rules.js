...
function begin_play(){
	...
	var tinfo = document.getElementById('turn_info');
	console.log(p1name.innerHTML);
	console.log(p2name.innerHTML);

	if(game_started()){
		alert("Already started. Please 𝗥𝗲𝘀𝗲𝘁 𝗣𝗹𝗮𝘆 to start again.")
	}

	else if((isEmpty(p1name.value) || isEmpty(p2name.value))){
		alert("Two player game, both the fields are mandatory");
		this.started = false;
		tinfo.innerHTML = "Game has not begin."
		p1name.disabled = false;
		p2name.disabled = false;
	}
	else{
		this.started = true;
		tinfo.innerHTML = "Turn for : <b>X</b>" ;
...
function reset_play(){
	var p1name = document.getElementById('player1_id');
	var p2name = document.getElementById('player2_id');
	var moveTxt = document.getElementById("move_text_id");
	var tinfo = document.getElementById('turn_info');
	p1name.value = "";
	p2name.value = "";
	moveTxt.value = "";
	tinfo = "Game has not begun.";
	this.started = false;
	p1name.disabled = false;
	p2name.disabled = false;
	this.turn=1;
...
*/
function play() {
	var playPos = document.getElementById('move_text_id');
	var tinfo = document.getElementById('turn_info');
	var daTable =document.getElementById('table');
	var z = false;
	var bdstate;
	if(!game_started()){
		...
		if(z == false){
			...
			if(whose_move() == 1){
				document.getElementById(playPos.value).innerHTML = "X";
				console.log("x")
				board_state[bdstate] = 1;
				tinfo.innerHTML = "Turn for : <b>O</b>";

				z = false;
			}
			else if(whose_move() == 0){
				document.getElementById(playPos.value).innerHTML = "O";
				console.log("y")
				board_state[bdstate] = 0;
				tinfo.innerHTML = "Turn for : <b>X</b>" ;
				z = false;
			}
			toggle_move();
			...
			else if((board_state[6]==board_state[4])&&(board_state[6]==board_state[2])){
				if(board_state[6]==1){
					alert("X is the Winner");
				}
				else if(board_state[6]==0){
